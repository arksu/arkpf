...
begin
  ...
  case typ[1] of
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Inc(PosInPkt,d);
          value:='skip' (* 'Пропускаем ' *) +inttostr(d)+'byte' (* ' байт(а)' *) ;
        end else
          value:='skip script' (* 'Пропускаем скрипт' *) ;
      end else
      begin
        d:=strtoint(name_);
        Inc(PosInPkt,d);
    ...
    else value:= 'unknownid' (* 'Неизвестный идентификатор -> ?(name_)!' *) ;
  end;
  Result:=value;
...
//        for i:=1 to StrToInt(tmp_value) do
        for i:=1 to val do
        begin
          rvDescryption.AddNL('              startb' (* '[Начало повторяющегося блока ' *) , 0, 0);
          ...
          rvDescryption.AddNL('              endb' (* '[Конец повторяющегося блока ' *) , 0, 0);
          rvDescryption.AddNL(inttostr(i)+'/'+end_block, 1, -1);
          rvDescryption.AddNL(']', 0, -1);
...
//    else
      Label1.Caption:='selected pck type : ' (* 'Выделенный пакет: тип - 0x' *) +IntToHex(cID, 2)+', '+PacketName+' size ' (* ', размер - ' *) +IntToStr(wSize);
    ...
    Inc(PosInIni);
    //Добавляем тип
    rvDescryption.AddNL('type ' (* 'Tип: ' *) , 11, 0);
//    if ((GlobalProtocolVersion=AION27) and (wSubId<>0))then // для Айон 2.7 двухбайтные ID
...
//    else
    ...
    rvDescryption.AddNL('size2' (* 'Pазмер: ' *) , 0, 0);
    rvDescryption.AddNL(IntToStr(wSize-2), 1, -1);
    rvDescryption.AddNL('+2', 2, -1);

    rvDescryption.AddNL('time ' (* 'Время прихода: ' *) , 0, 0);
