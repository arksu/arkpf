...
type
    ...
    public
        ...
        procedure onMagicEffectIcons(pck : AnsiString);
...
begin
    ...
    SkillsReady := false;
    if myEngine.Me.HaveSkill(271) then begin
        AddSkill( 271, 'Warrior' ); SkillsReady := true;
    end;
    if myEngine.Me.HaveSkill(274) then begin
        AddSkill( 274, 'Fire' ); SkillsReady := true;
    end;
    if myEngine.Me.HaveSkill(275) then begin
        AddSkill( 275, 'Fury' ); SkillsReady := true;
    end;
    if myEngine.Me.HaveSkill(310) then begin
        AddSkill( 310, 'Vampire' ); SkillsReady := true;
    end;
    if myEngine.Me.HaveSkill(366) then begin
        AddSkill( 366, 'Shadow' ); SkillsReady := true;
    end;
    if myEngine.Me.HaveSkill(915) then begin
        AddSkill( 915, 'Berserker' ); SkillsReady := true;
    end;
    if myEngine.Me.HaveSkill(276) then begin
        AddSkill( 276, 'Concentration' ); SkillsReady := true;
    end;
    if myEngine.Me.HaveSkill(272) then begin
        AddSkill( 272, 'Inspiration' ); SkillsReady := true;
    end;
    LoadOptions;
end;
...
begin
    ...
    if sn = 'bb_sm_disable' then begin
        myEngine.botSay( 'shadow mode OFF');
        ShadowMode := false;
        TimerStop(1);
        ShowOptions;
    end;

    if sn = 'bb_sm_enable' then begin
        myEngine.botSay( 'shadow mode ON');
        ShadowMode := true;
...
begin
    ...
    OptionsFname := AppPath + 'ini\auto_bd_'+core.getLinkedCharName(Self)+'.txt';
...
begin
    ...
    if (ch = 3) and (msg = 'ds') or (msg = 'bd') then begin
        StartDS;
    end;

    if (ch = 3) and (msg = 'sm') then begin
        myEngine.Me.UseSkill(366);
    end;
    if (ch = 3) and (msg = 'sm+') then begin
        ShadowMode := true;
    ...
    if (ch = 3) and (msg = 'bers') then begin
        myEngine.Me.UseSkill(915);
    end;
    if (ch = 3) and (msg = 'bd+') then begin
        Active := True;
        myEngine.botSay('bd on');
    end;
    if (ch = 3) and (msg = 'bd-') then begin
        Active := false;
        ds_index := 1000;
        myEngine.botSay('bd off');
    end;
...
begin
    ...
    if (msg = 'a') then begin
        ...
        onSkillsList;
        LoadOptions;
        myEngine.botSay( 'bd activated' );
    end;

    if (msg = 'd') then begin
        Active := False;
        pck := '';
        myEngine.botSay( 'bd deactivated' );
    end;
end;
...
procedure TPluginImpl.onMagicEffectIcons(pck: AnsiString);
var
    p : TPacket;
    a, c, i : Integer;
begin
    ...
    for i := 1 to c do begin
        if p.ReadD = 366 then begin
            p.ReadH;
            a := p.ReadD;
            myEngine.botSay( 'sm duration='+inttostr(a));
            if (a < 10) and active then begin
                myEngine.Me.UseSkill(366);
                TimerStop(1);
            end else begin
                TimerStart(1, (a-9) * 1000 );
            end;
        end else begin
            p.ReadH;
            p.ReadD;
...
begin
    ss := '<html><body>'+
    '<title>Auto BD by arksu ['+LinkedCharName+']</title>'+
    '<table>';
    ...
    if ShadowMode then begin
        col := 'FFFFFF';
        ss :=ss +
        '<tr><td height=38><img src="Icon.skill0366" width=32 height=32>'+
        '</td><td width=80><font color="'+col+'">Shadow mode ON</font></td>';

            ss := ss+'<td><button action="bypass bb_sm_disable" value="OFF"'+
            'width=70 height=25 back="L2UI_CT1.Button_DF_Down" fore="L2UI_CT1.Button_DF"></td></tr>'
    end
    else begin
        col := '606066';
        ss :=ss +
        '<tr><td height=38><img src="Icon.skill0366" width=32 height=32>'+
        '</td><td width=80><font color="'+col+'">Shadow mode OFF</font></td>';
            ss := ss+'<td><button action="bypass bb_sm_enable" value="ON"'+
            'width=70 height=25 back="L2UI_CT1.Button_DF_Down" fore="L2UI_CT1.Button_DF"></td></tr>'
    end;
    ss := ss + '</table>';
